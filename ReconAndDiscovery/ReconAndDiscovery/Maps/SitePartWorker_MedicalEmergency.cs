	...
	public class SitePartWorker_MedicalEmergency : SitePartWorker
	{
		public override void PostMapGenerate(Map map)
		{
			...
			if (RCellFinder.TryFindRandomCellNearTheCenterOfTheMapWith((IntVec3 x) => x.Standable(map) && x.Fogged(map) && GridsUtility.GetRoom(x, map, 6).CellCount >= 2, map, out baseCenter))
			{
				...
				{
					IntVec3 intVec;
					if (RCellFinder.TryFindRandomCellNearTheCenterOfTheMapWith((IntVec3 x) => x.Standable(map) && x.Fogged(map) && GridsUtility.GetRoom(x, map, 6).CellCount >= 2, map, out intVec))
					{
